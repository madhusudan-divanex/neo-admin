...
function LabLandingFull() {
  const [activeTab, setActiveTab] = useState("first");

  return (
    <div className="main-content flex-grow-1 p-3 overflow-auto">
      <h3 className="gradient-text mb-3">Lab Landing Page</h3>
      ...
      {/* 👇 Tab Content */}
      <div className="mt-4">
        {activeTab === "first" && <FirstLabPage />}
        {activeTab === "second" && <SecondLabPage />}
        {activeTab === "third" && <ThirdLabPage />}
        {activeTab === "fourth" && <FourthLabPage />}
        {activeTab === "fiveth" && <FivethLabPage />}
        {activeTab === "six" && <SixLabPage />}
        {activeTab === "seven" && <SevenLabPage />}
      </div>
    </div>
  );
}

export default LabLandingFull;