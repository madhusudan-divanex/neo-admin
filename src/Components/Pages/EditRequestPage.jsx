...
function EditRequestPage({ type, title }) {
  ...
  const fetchList = async () => {
    try {
      ...
      const res = await api.get("/api/admin/edit-requests", {
        params: { type, status: statusFilter, page, limit }
      });
  ...
  const getImg  = (item) => {
    ...
    if (u.profileImage) return `${IMAGE_BASE_URL}/uploads/${type}/${u.profileImage}`;
    return type === "doctor" ? "/doctor-avatr.png" : "/admin-tb-logo.png";
  };
