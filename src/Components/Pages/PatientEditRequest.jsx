  ...
  const fetchList = async () => {
    try {
      ...
      const res = await api.get("/api/admin/edit-requests", {
        params: { type: "patient", status: statusFilter, page, limit, search }
      });
      ...
      <div className="main-content flex-grow-1 p-3 overflow-auto">
        <div className="row mb-3">
          <div className="d-flex align-items-center justify-content-between">
            <div>
              <h3 className="innr-title mb-2 gradient-text">Patient Edit Request</h3>
              <div className="admin-breadcrumb">

                <nav aria-label="breadcrumb">
                  <ol className="breadcrumb custom-breadcrumb">
                    <li className="breadcrumb-item"><a href="#" className="breadcrumb-link">Dashboard</a></li>
                    <li className="breadcrumb-item active">Patient Edit Request</li>
                  </ol>
                </nav>
              </div>
            </div>
                ...
                <div className="dropdown-menu dropdown-menu-end user-dropdown tble-action-menu p-3" style={{ minWidth: 160 }}>
                  ...
                  {["all", "pending", "approved", "rejected"].map(s => (
                    <div className="form-check new-custom-check" key={s}>
                      <input className="form-check-input" type="radio" name={`er-patient`} id={`er-patient-${s}`}
                        checked={statusFilter === s} onChange={() => { setStatusFilter(s); setPage(1); }} />
                      <label className="form-check-label text-capitalize" htmlFor={`er-patient-${s}`}>{s}</label>
                    </div>
                  ))}
                </div>
              </div>
                      ...
                      <tr key={item._id}>
                        ...
                        <td>
                          <ul className="ad-info-list mt-1">
                            {item?.patientId?.contactNumber && <li className="ad-info-item" style={{ fontSize: 12 }}>Mobile : {item?.patientId?.contactNumber}</li>}
                            {item?.patientId?.email && <li className="ad-info-item" style={{ fontSize: 12 }}>Email : {item?.patientId?.email}</li>}
                          </ul>
                        </td>
                        <td>
                          {calculateAge(item?.about?.dob)}
                        </td>
                        <td>
                          {fmt(item?.createdAt)}
                        </td>
